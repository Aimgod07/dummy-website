const Samples = () => {
  return (
    <>
    <div className="samples" style={{alignItems: 'center', textAlign: 'center',marginLeft: '100px', marginRight: '10px',marginTop:"60px"}}>
      <div class="grid grid-flow-col grid-rows-4 gap-8">
  <div>
    <a href="../assets/image.png" >
    <img src="../assets/aeroflot.png" alt="airline" style={{maxBlockSize:"50px"}}/>
    </a>
    </div>
  <div><a href="../assets/image.png" >
  <img src="../assets/a2.png" alt="airline"style={{maxBlockSize:"50px"}}/></a>
  </div>
  <div><a href="../assets/image.png" >
  <img src="../assets/a3.png" alt="airline"style={{maxBlockSize:"50px"}}/></a>
  </div> 
  <div><a href="../assets/image.png" >
  <img src="../assets/a4.png" alt="airline"style={{maxBlockSize:"50px"}}/></a>
  </div>
  <div>
    <a href="../assets/image.png" ><img src="../assets/a5.png" alt="airline"style={{maxBlockSize:"50px"}}/></a>
  </div>
  <div>
    <a href="../assets/image.png" ><img src="../assets/a6.png" alt="airline"style={{maxBlockSize:"50px"}}/></a>
  </div>
  <div>
    <a href="../assets/image.png" ><img src="../assets/a7.png" alt="airline"style={{maxBlockSize:"50px"}}/></a>
  </div>
  <div>
   <a href="../assets/image.png" ><img src="../assets/a8.png" alt="airline"style={{maxBlockSize:"50px"}}/></a>
  </div>
  <div>
    <a href="../assets/image.png" ><img src="../assets/a9.png" alt="airline"style={{maxBlockSize:"50px"}}/></a>
  </div>
  <div><a href="../assets/image.png" >
  <img src="../assets/a10.png" alt="airline"style={{maxBlockSize:"50px"}}/></a>
  </div>
  <div><a href="../assets/image.png" >
  <img src="../assets/a11.png" alt="airline"style={{maxBlockSize:"50px"}}/></a>
  </div>
  <div><a href="../assets/image.png" >
  <img src="../assets/a12.png" alt="airline"style={{maxBlockSize:"50px"}}/></a>
  </div>
   </div>

    </div>
    <a href="/buynow" >
       <div className="box" style={{ backgroundColor: 'lightblue', padding: '20px', maxWidth: '200px',  textAlign: 'center', borderRadius: '8px', fontSize: '20px'}}>
          Buy Now
       </div>
       </a>
    </>
  );
}
export default Samples;